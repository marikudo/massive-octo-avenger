...
class sections extends crackerjack{
	...
	public function index($id = false){
		$data['active_module'] = "sections";
		$data['sections'] = $this->crud->read('SELECT *,_course.course FROM _section INNER JOIN _course ON _course.course_id = _section.course_id WHERE _section.user_id=:id ORDER BY _section.section',array('id'=>$this->session->_get('uid')),'obj');
		...
		$this->load->template('xadmin/sections',25,$data);
	}
	...
	public function edit($id =false){
		...
		$result = $this->form->post('btn-submit');
			...
			$data['result'] = $this->crud->read('SELECT * FROM _section WHERE section_id = :id',array(':id'=>$id),'assoc');
			$data['action'] = 'Edit';
			$data['course'] = $this->crud->read('SELECT * FROM _course WHERE user_id=:id',array('id'=>$this->session->_get('uid')),'obj');
		$this->load->template('xadmin/sections_',$this->acl->isAllowed(25),$data);	
	}
	...
	public function validate($id = false){
			if($id[0]=="ajax=true"){
				...
				if(isset($_REQUEST['section'])){
					$res =  $this->crud->read('SELECT count(*) as count FROM _section WHERE section=:section AND course_id = :cid AND user_id=:id',array(':section'=>$_REQUEST['section'],':id'=>$this->session->_get('uid'),'cid'=>$_REQUEST['course_id']),'assoc');
